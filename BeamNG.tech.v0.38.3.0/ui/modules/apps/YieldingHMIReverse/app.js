angular.module('beamng.apps')
  .directive('yieldingHmiReverse', ['$timeout', '$window', function ($timeout, $window) {
    return {
      templateUrl: '/ui/modules/apps/YieldingHMIReverse/app.html',
      replace: true,
      restrict: 'EA',
      link: function (scope, element) {
        ...
        var persistentKey = 'yieldingHmiReversePersistent'
        ...
        _loadAudioBuffer('yielding_reverse.mp3', 'yielding_reverse')
        ...
        function updateNav(distanceMeters) {
          ...
          bngApi.engineLua(lua, function (data) {
            ...
            scope.$evalAsync(function () {
              if (navLogCount < 3) {
                ...
                navLoggedOnce = true
                try {
                  console.warn('[YieldingHMIReverse] navigation data sample #' + navLogCount, data)
                } catch (e) { }
              }
        ...
        scope.$on('streamsUpdate', function (event, streams) {
          ...
          scope.$evalAsync(function () {
            ...
            scope.state.manualDistanceText = formatDistance(streams.electrics.trip)
            ...
            var detectYaw = streams.electrics.detect_yaw
            if (detectYaw === undefined) detectYaw = streams.electrics.charger_yaw
            if (detectYaw !== undefined) {
              scope.state.detectYaw = Math.round(detectYaw)
            }

            var detectDist = streams.electrics.detect_dist
            if (detectDist === undefined) detectDist = streams.electrics.charger_dist
            if (detectDist !== undefined) {
              scope.state.detectDistText = Math.round(detectDist) + 'm'
            }
            ...
            if (scope.state.batteryPercent <= 10) {
              if (!scope.state.isCritical) {
                ...
                console.log('[YieldingHMIReverse] Guidance Compass Activated!')
              }
            }
