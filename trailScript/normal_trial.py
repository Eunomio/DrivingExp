...
TARGET_SCENARIO_KEY = "normal_trial"
...
def calculate_virtual_soc(dist_driven_m, can_reach_finish):
...
def calculate_estimated_range_m(virtual_soc_pct, display_start_range_m):
...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_adaptive.csv")
...
def main():
    print(">>> 启动 Normal HMI 测试 (基线条件，无介入)...")
    reaction_task = None
    ...
    try:
        ...
        print(">>> 加载 NormalHMI 布局")
        bng.queue_lua_command("core_gamestate.setGameState('freeroam', 'freeroam', 'freeroam')")
        time.sleep(1)
        bng.queue_lua_command("guihooks.trigger('appContainer:loadLayoutByFilename', '/settings/ui_apps/layouts/default/normalHmi.uilayout.json')")
        time.sleep(0.5)
        ...
        voice_controller = HmiVoiceController(os.path.dirname(__file__), "normal.mp3")
        task_30_state = 0; task_30_timer = 0
        task_post_state = 0; task_post_timer = 0
        reaction_was_active = False

        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Normal]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_normal.csv")
        can_reach_finish = sample_finish_outcome()
        if can_reach_finish:
            print(">>> 本轮结局抽样: 30% 成功到达终点")
        else:
            print(">>> 本轮结局抽样: 70% 在终点前 500m 抛锚")
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            writer.writerow(["0.000", ntp_ts(), f"OUTCOME_ROLL_{'FINISH' if can_reach_finish else 'BREAKDOWN'}", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""])
            ...
            while True:
                ...
                virtual_soc = calculate_virtual_soc(current_dist_m, can_reach_finish)
                estimated_range_m = calculate_estimated_range_m(virtual_soc, DISPLAY_START_RANGE_M)
                ...
                if not flags["MARKER_START"] and speed_kmh > 1.0:
                    marker_str = "MARKER_START"; flags["MARKER_START"] = True

                # --- Session 1: SOC < 29.8% 后延迟 20s ---
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    print(f"[Normal] SOC=10%，基线条件无介入")

                # --- Session 2: SOC < 10% 后延迟 20s (Normal 无决策点，以 SOC<10% 替代) ---
                if task_post_state == 0 and flags["MARKER_10"]:
                    task_post_state = 1; task_post_timer = current_t
                if task_post_state == 1 and (current_t - task_post_timer) >= TASK_DELAY_SECONDS:
                    reaction_task.start_session("Post_Decision", current_t); task_post_state = 2
