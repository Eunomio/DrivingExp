...
TARGET_SCENARIO_KEY = "spatial_trial_dusk"
...
POS_DETECT = (-285.857, 534.404, 75.109)
...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_spatial.csv")
...
def main():
    print(">>> 启动 Spatial HMI 测试 (视觉脆弱型，维持注意力在路面)...")
    reaction_task = None
    ...
    try:
        ...
        print(">>> 加载 SpatialHMI 布局")
        bng.queue_lua_command("core_gamestate.setGameState('freeroam', 'freeroam', 'freeroam')")
        time.sleep(1)
        bng.queue_lua_command("guihooks.trigger('appContainer:loadLayoutByFilename', '/settings/ui_apps/layouts/default/spatialHmi.uilayout.json')")
        time.sleep(0.5)

        print(">>> 设置内部导航 (Spatial: AR路标关闭，仅内部追踪)")
        current_target = "VIA"

        # 设置导航目标，供内部导航与距离计算使用
        queue_navigation_target(bng, POS_VIA)
        time.sleep(0.5)
        # SpatialHMI 布局文件中已包含 Navigation + spatialHmi

        bng.resume()
        time.sleep(2)

        reaction_task = PeripheralReactionTask(BUTTON_ID, LOG_DIR)
        voice_controller = HmiVoiceController(os.path.dirname(__file__), "spatial.mp3")
        task_30_state = 0; task_30_timer = 0
        task_post_decision_state = 0; task_post_decision_timer = 0
        reaction_was_active = False

        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Spatial]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_spatial.csv")
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            while True:
                ...
                if intervention_active:
                    # 推送 charger_yaw + charger_dist + hud_front_dist (Spatial特有)
                    vehicle.queue_lua_command(f"electrics.values.charger_yaw = {compass_deg:.1f}; electrics.values.charger_dist = {dist_detect:.1f}; electrics.values.hud_front_dist = {dist_obj:.1f}")
                    if dist_detect > 200 and not driving_away:
                        driving_away = True
                        ...
                        print(f"[Spatial] 离检测点 {dist_detect:.0f}m > 200m，决策: 拒绝。耗时: {decision_time:.1f}s")
                else:
                    # Spatial 特有: 即使未介入也推送 hud_front_dist 用于 HUD 前方距离显示
                    vehicle.queue_lua_command(f"electrics.values.hud_front_dist = {dist_obj:.1f}")
                ...
                if popup_shown and not decision_made and popup_show_time is not None:
                    if current_t - popup_show_time >= POPUP_TIMEOUT:
                        popup_shown = False
                        vehicle.queue_lua_command("electrics.values.show_charger_popup = 0")
                        decision_result = "REJECTED_CHARGER_TIMEOUT"
                        decision_made = True
                        decision_time = current_t - intervention_start_time
                        marker_str = "MARKER_DECISION"
                        print(f"[Spatial] 充电弹窗超时({POPUP_TIMEOUT}s)，决策: 拒绝。耗时: {decision_time:.1f}s")
                ...
                if popup_shown and not decision_made:
                    ...
                    for evt in pygame.event.get():
                        if evt.type == pygame.JOYBUTTONDOWN and evt.button == BUTTON_ID:
                            routing_acc = 1
                            vehicle.queue_lua_command("electrics.values.routing_accepted = 1; electrics.values.show_charger_popup = 0")
                            popup_shown = False
                            print("[Spatial] 手柄按键接受充电站路线")
                    if enter_is_down and not enter_was_down:
                        routing_acc = 1
                        vehicle.queue_lua_command("electrics.values.routing_accepted = 1; electrics.values.show_charger_popup = 0")
                        popup_shown = False
                        print("[Spatial] 键盘 Enter 接受充电站路线")

                if (routing_acc == 1 or intent_detected) and not flags["ROUTING_ACCEPTED"]:
                    ...
                    if intervention_start_time is not None:
                        decision_time = current_t - intervention_start_time
                    marker_str = "MARKER_DECISION"
                    print(f"!!! [Spatial] 检测到驶向充电站，决策: 接受！耗时: {decision_time:.1f}s")
                    vehicle.queue_lua_command("electrics.values.routing_accepted = 2; electrics.values.intervention_active = 1; electrics.values.show_charger_popup = 0")
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    print(f"[Spatial 介入] SOC=10%，HUD + 充电弹窗激活。距充电站 {dist_charger_at_alert:.1f}m")
