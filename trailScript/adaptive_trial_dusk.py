...
TARGET_SCENARIO_KEY = "adaptive_trial_dusk"
...
def main():
    print(">>> 启动 Adaptive HMI 测试 (认知敏感型，降低信息密度)...")
    reaction_task = None
    ...
    try:
        ...
        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Adaptive]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_adaptive.csv")
            ...
            while True:
                ...
                if intervention_active:
                    ...
                    if dist_detect > 200 and not driving_away and not decision_made:
                        ...
                        current_target = "VIA"
                        print(f"[Adaptive] 离检测点 {dist_detect:.0f}m > 200m，决策: 拒绝。耗时: {decision_time:.1f}s")
                ...
                if popup_shown and not decision_made and popup_show_time is not None:
                    if current_t - popup_show_time >= POPUP_TIMEOUT:
                        ...
                        if intervention_start_time is not None:
                            decision_time = current_t - intervention_start_time
                        marker_str = "MARKER_DECISION"
                        print(f"[Adaptive] 弹窗超时({POPUP_TIMEOUT}s)，用户未取消，接受改道。耗时: {decision_time:.1f}s")
                ...
                if popup_shown and not decision_made:
                    ...
                    for evt in pygame.event.get():
                        if evt.type == pygame.JOYBUTTONDOWN and evt.button == BUTTON_ID:
                            ...
                            restore_default_route(bng)
                            current_target = "VIA"
                            print(f"[Adaptive] 手柄按键取消改道，决策: 拒绝。耗时: {decision_time:.1f}s")
                    if enter_is_down and not enter_was_down and not decision_made:
                        ...
                        restore_default_route(bng)
                        current_target = "VIA"
                        print(f"[Adaptive] 键盘 Enter 取消改道，决策: 拒绝。耗时: {decision_time:.1f}s")

                # --- UI 侧取消检测 ---
                cancel_flag = s_elec.get('cancel_charger', 0)
                if cancel_flag == 1 and popup_shown and not decision_made:
                    popup_shown = False
                    vehicle.queue_lua_command("electrics.values.show_charger_popup = 0; electrics.values.cancel_charger = 0")
                    decision_result = "REJECTED_CHARGER"
                    decision_made = True
                    if intervention_start_time is not None:
                        decision_time = current_t - intervention_start_time
                    ...
                    restore_default_route(bng)
                    current_target = "VIA"
                    print(f"[Adaptive] UI 按钮取消改道，决策: 拒绝。耗时: {decision_time:.1f}s")
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    print(f"[Adaptive 介入] SOC=10%，自动导航至充电站 + 弹窗。距充电站 {dist_charger_at_alert:.1f}m")
