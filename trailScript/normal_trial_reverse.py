...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_normal_reverse.csv")
...
def main():
    print(">>> 启动 Normal HMI 测试 (反向路线，基线条件，无介入)...")
    ...
    try:
        ...
        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Normal_Rev]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_normal_reverse.csv")
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            while True:
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    print(f"[Normal 反向] SOC=10%，基线条件无介入")
