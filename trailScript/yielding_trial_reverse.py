...
TARGET_SCENARIO_KEY = "yielding_trial_reverse"
...
TARGET_CHARGER = (-497.187, 140.643, 100.553)  # 充电站虚拟坐标（不变）
POS_DETECT = (-285.857, 534.404, 75.109)       # 与正向一致的检测点坐标
...
def make_safe_subject_id(raw_value):
    value = (raw_value or "").strip()
    if not value:
        return f"session_{int(time.time())}"

    # If terminal command text was accidentally sent to stdin, ignore it.
    lowered = value.lower()
    if "python.exe" in lowered or lowered.endswith(".py") or "beamngtech" in lowered:
        return f"session_{int(time.time())}"
...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_yielding_reverse.csv")
...
def main():
    ...
    try:
        ...
        print(">>> 加载 YieldingHMIReverse 布局")
        bng.queue_lua_command("core_gamestate.setGameState('freeroam', 'freeroam', 'freeroam')")
        time.sleep(1)
        bng.queue_lua_command("guihooks.trigger('appContainer:loadLayoutByFilename', '/settings/ui_apps/layouts/default/yieldingHmiReverse.uilayout.json')")
        time.sleep(0.5)
        ...
        voice_controller = HmiVoiceController(os.path.dirname(__file__), "yielding_reverse.mp3")
        task_30_state = 0; task_30_timer = 0
        task_post_decision_state = 0; task_post_decision_timer = 0
        reaction_was_active = False

        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        raw_subject_id = input("请输入被试ID [例如: S01_Yielding_Rev]: ")
        subject_id = make_safe_subject_id(raw_subject_id)
        if subject_id != (raw_subject_id or "").strip():
            print(f">>> 被试ID已规范化为: {subject_id}")
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_yielding_reverse.csv")
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            while True:
                ...
                # --- 计算从当前车辆位置到 POS_DETECT 的罗盘方向 ---
                dx_detect = POS_DETECT[0] - pos[0]
                dy_detect = POS_DETECT[1] - pos[1]
                compass_deg = math.degrees(math.atan2(dx_detect, dy_detect))
                if compass_deg < 0:
                    compass_deg += 360
                ...
                if intervention_active:
                    vehicle.queue_lua_command(
                        f"electrics.values.detect_yaw = {compass_deg:.1f}; "
                        f"electrics.values.detect_dist = {dist_detect:.1f}; "
                        f"electrics.values.charger_yaw = {compass_deg:.1f}; "
                        f"electrics.values.charger_dist = {dist_detect:.1f}"
                    )
                    ...
                    if dist_detect > DETECT_EXIT_DIST and not driving_away and not decision_made:
                        ...
                        current_target = "END"
                        print(f"[Yielding] 离检测点 {dist_detect:.0f}m > 200m，禁用意图识别。决策耗时: {decision_time:.1f}s")

                s_elec = vehicle.sensors['electrics']

                # --- 交互核心：监听隐式意图 (Intent Recognition) ---
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    entered_detection_zone = False
                    driving_away = False
                    print(f"[W3 反向陪伴介入] SOC=10%，距充电站 {dist_charger_at_alert:.1f}m。等待驾驶员意图...")
