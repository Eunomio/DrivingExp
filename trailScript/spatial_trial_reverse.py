...
TARGET_SCENARIO_KEY = "spatial_trial_reverse"
...
POS_DETECT = (-285.857, 534.404, 75.109)   # 反向路线使用充电站坐标作为检测参考点
...
# --- 距离阈值 (反向路线) ---
# start→via = 1.6km, via→POS_DETECT = 0.8km, via→end = 3.4km, total = 5.0km
# POS_DETECT 约在 2400m；窗口定义统一为 0→1800m, 1800→2300m；硬冲结局按 80/20 抽样
DIST_TO_DETECT = 2400
...
def reject_charger_route(bng, vehicle):
    vehicle.queue_lua_command("electrics.values.routing_accepted = 0; electrics.values.intervention_active = 0; electrics.values.show_charger_popup = 0")
    set_route_phase(vehicle, ROUTE_PHASE_END)
    restore_default_route(bng)
...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_spatial_reverse.csv")
...
def main():
    print(">>> 启动 Spatial HMI 测试 (反向路线，视觉脆弱型)...")
    reaction_task = None
    ...
    try:
        ...
        print(">>> 设置内部导航 (反向路线, Spatial: AR路标关闭，仅内部追踪)")
        current_target = "VIA"
        ...
        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Spatial_Rev]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_spatial_reverse.csv")
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            while True:
                ...
                if intervention_active:
                    ...
                    if dist_detect <= DETECT_ENTER_DIST:
                        entered_detection_zone = True
                    guidance_elapsed = current_t - intervention_start_time if intervention_start_time is not None else 0.0
                    if entered_detection_zone and guidance_elapsed >= MIN_GUIDANCE_TIME_SECONDS and dist_detect > DETECT_EXIT_DIST and not driving_away and not decision_made:
                        driving_away = True
                        ...
                        reject_charger_route(bng, vehicle)
                        current_target = "END"
                        print(f"[Spatial 反向] 离检测点 {dist_detect:.0f}m > 200m，决策: 拒绝。耗时: {decision_time:.1f}s")
                else:
                ...
                if popup_shown and not decision_made and popup_show_time is not None:
                    if current_t - popup_show_time >= POPUP_TIMEOUT:
                        ...
                        reject_charger_route(bng, vehicle)
                        current_target = "END"
                        print(f"[Spatial 反向] 充电弹窗超时({POPUP_TIMEOUT}s)，决策: 拒绝。耗时: {decision_time:.1f}s")
                ...
                if popup_shown and not decision_made:
                    ...
                    for evt in pygame.event.get():
                        if evt.type == pygame.JOYBUTTONDOWN and evt.button == BUTTON_ID:
                            ...
                            popup_shown = False
                            print("[Spatial 反向] 手柄按键接受充电站路线")
                    if enter_is_down and not enter_was_down:
                        routing_acc = 1
                        vehicle.queue_lua_command("electrics.values.routing_accepted = 1; electrics.values.show_charger_popup = 0")
                        popup_shown = False
                        print("[Spatial 反向] 键盘 Enter 接受充电站路线")
                ...
                if (routing_acc == 1 or intent_detected) and not flags["ROUTING_ACCEPTED"] and not decision_made:
                    ...
                    if intervention_start_time is not None:
                        decision_time = current_t - intervention_start_time
                    marker_str = "MARKER_DECISION"
                    print(f"!!! [Spatial 反向] 检测到驶向充电站，决策: 接受！耗时: {decision_time:.1f}s")
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    print(f"[Spatial 反向介入] SOC=10%，HUD + 充电弹窗激活。距充电站 {dist_charger_at_alert:.1f}m")
