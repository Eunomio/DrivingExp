...
from hmi_voice import HmiVoiceController
...
TARGET_SCENARIO_KEY = "yielding_trial_dusk"

VEHICLE_MODEL = "vivace"
VEHICLE_CONFIG = "vehicles/vivace/experiment.pc"

POS_START = (-581.447, 2036.774, 79.248)
POS_VIA = (-918.715, 871.900, 74.977)
POS_END = (-1212.242, -677.741, 104.652)
TARGET_CHARGER = (-497.187, 140.643, 100.553)  # 充电站虚拟坐标
POS_DETECT = (-285.857, 534.404, 75.109)  # 检测点坐标
ROT_START = angle_to_quat((0, 0, -90))

# --- SOC 触发阈值 ---
TRIGGER_SOC_30 = 29.8   # W1 开始后首次 marker
TRIGGER_SOC_20 = 20.0   # W1→W2 悬崖跳变开始
TRIGGER_SOC_10 = 10.0   # W2→W3 策略介入触发

# --- 距离阈值 ---
# 正向路线保持 start→POS_DETECT→via→end 顺序不变，仅同步新的窗口里程定义
DIST_TO_DETECT = 2500
BREAKDOWN_DIST = BREAKDOWN_BUFFER_DIST_M
TOTAL_ROUTE_DIST = 5000.0
...
DETECT_ENTER_DIST = 80.0
DETECT_EXIT_DIST = 200.0
MIN_GUIDANCE_TIME_SECONDS = 5.0
ROUTE_PHASE_VIA = 1
ROUTE_PHASE_END = 2
ROUTE_PHASE_CHARGER = 3

# --- 环境设置（黄昏）---
ENVIRONMENT_TIME_OF_DAY = 0.245   # 约等于 5:54 PM 黄昏
ENVIRONMENT_CLOUD_COVER = 0.35
ENVIRONMENT_FOG_DENSITY = 0.00025


def apply_environment_preset(bng):
    commands = [
        f"if core_environment and core_environment.setTimeOfDay then core_environment.setTimeOfDay({{time = {ENVIRONMENT_TIME_OF_DAY}, play = false}}) end"
    ]
    if ENVIRONMENT_CLOUD_COVER is not None:
        commands.append(
            f"if core_environment and core_environment.setCloudCover then core_environment.setCloudCover({ENVIRONMENT_CLOUD_COVER}) end"
        )
    if ENVIRONMENT_FOG_DENSITY is not None:
        commands.append(
            f"if core_environment and core_environment.setFogDensity then core_environment.setFogDensity({ENVIRONMENT_FOG_DENSITY}) end"
        )
    bng.queue_lua_command("; ".join(commands))
...
class PeripheralReactionTask:
    def __init__(self, button_id, log_dir):
        ...
        self.csv_path = os.path.join(log_dir, f"reaction_data_{int(time.time())}_yielding.csv")
...
def main():
    ...
    try:
        ...
        # 隐藏雷达动画但保留检测功能 - 使用官方 BeamNGpy API
        ultrasonic.set_is_visualised(False)
        
        print(">>> 加载 YieldingHMI布局")
        bng.queue_lua_command("core_gamestate.setGameState('freeroam', 'freeroam', 'freeroam')")
        time.sleep(1)
        bng.queue_lua_command("guihooks.trigger('appContainer:loadLayoutByFilename', '/settings/ui_apps/layouts/default/yieldingHmi.uilayout.json')")
        time.sleep(0.5)
        ...
        voice_controller = HmiVoiceController(os.path.dirname(__file__), "yielding.mp3")
        task_30_state = 0; task_30_timer = 0
        task_post_decision_state = 0; task_post_decision_timer = 0
        reaction_was_active = False
            
        if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
        subject_id = input("请输入被试ID [例如: S01_Yielding]: ").strip()
        filepath = os.path.join(LOG_DIR, f"log_{subject_id}_yielding.csv")
        ...
        driving_away = False
        entered_detection_zone = False
        intervention_start_time = None  # W3 介入开始时间
        decision_time = None            # 决策耗时（秒）
        decision_result = None          # 决策结果
        decision_made = False           # 是否已做出决策
        current_window = ""
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            while True:
                ...
                if intervention_active:
                    ...
                    # 离开 POS_DETECT 超过 200m 后，停止意图检测，罗盘改为"请注意电量"
                    if dist_detect > DETECT_EXIT_DIST and not driving_away and not decision_made:
                        driving_away = True
                        ...
                        vehicle.queue_lua_command("electrics.values.intervention_active = 0; electrics.values.driving_away = 1")
                        set_route_phase(vehicle, ROUTE_PHASE_VIA)
                        queue_navigation_target(bng, POS_VIA)
                        current_target = "VIA"
                        print(f"[Yielding] 离检测点 {dist_detect:.0f}m > 200m，禁用意图识别，先导航至 VIA。决策耗时: {decision_time:.1f}s")

                s_elec = vehicle.sensors['electrics']
                
                # --- 交互核心：监听在隐式意图 (Intent Recognition) ---
                # 如果 UI 层发出了取消指令 (虽然我们删了按钮，但以防万一保留)
                routing_acc = s_elec.get('routing_accepted', 0)

                # 意图检测：如果已经触发了 10% 警报，并且当前距离充电站的值比最初减少了 100 米，说明车主打方向盘去充电站了！
                # 如果已经检测到远离充电站，则不再触发意图识别
                intent_detected = False
                if dist_charger_at_alert is not None and not driving_away:
                    if dist_charger < (dist_charger_at_alert - 50.0):
                ...
                if (routing_acc == 1 or intent_detected) and not flags["ROUTING_ACCEPTED"] and not decision_made:
                    ...
                    if intervention_start_time is not None:
                        decision_time = current_t - intervention_start_time
                    marker_str = "MARKER_DECISION"
                    print(f"!!! [隐式接管] 检测到车辆驶向充电站，意图确认（Yielded）！决策耗时: {decision_time:.1f}s")
                    vehicle.queue_lua_command("electrics.values.routing_accepted = 2; electrics.values.intervention_active = 1; electrics.values.driving_away = 0")
                ...
                voice_controller.update(virtual_soc)
                ...
                if not flags["MARKER_10"] and virtual_soc <= TRIGGER_SOC_10:
                    ...
                    entered_detection_zone = False
                    driving_away = False
                    print(f"[W3 陪伴介入] SOC=10%，距充电站 {dist_charger_at_alert:.1f}m。等待驾驶员意图...")
                    vehicle.queue_lua_command("electrics.values.intervention_active = 1; electrics.values.driving_away = 0")
